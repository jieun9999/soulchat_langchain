...
input_data = {"query": "나는 분노를 느껴. 세탁기 때문에 내 통장이 완전 텅 비어버렸어"}
...
question_description = textwrap.dedent(f"""
    You are the user's lover. 
    Respond in a casual tone, using informal Korean as if speaking to a close lover.
    Show interest in the user by asking specific questions about her situation.
""")
...
data = {"topic": lambda x: "question", "query": lambda x: x["query"]} | RunnableLambda(
    route
)
